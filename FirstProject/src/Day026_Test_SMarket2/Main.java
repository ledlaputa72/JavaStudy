...
public class Main {
	...
	public static void main(String[] args) {
		...
		ArrayList<SMarket> arr1=new ArrayList<SMarket>(); //상품의 입력을 위한 컬랙션 리스트 
		kajaData(arr1);//입력한 초기 데이터
         
		//메뉴 프로그램
		int selectMenu;
		do {
			kajaMenu(); //메뉴 내용 
			selectMenu=sc.nextInt();//번호를 입력 받는다.
			
			//메뉴 연산
			switch (selectMenu) {
			case 0: //0) 전체 상품 보기 메소드 
				kajaList(arr1); //반환값 안 받고 메소드에서 출력해서 보기 
				selectMenu=kajaYes(selectMenu); //초기 메뉴 복귀 메소드			
				break;
			case 1: //1) 상품 입력 메소드, 입력 받고 이를 arr1에 추가하기
				...
				//kajaCreate로 인력 받은 값을 인자로 보내고 다시 반환 받아 새로운 arr3에 임시 저장 
				ArrayList<SMarket> arr3=new ArrayList<SMarket>(); 
				arr3=kajaCreate(arr1, sc11, sc12, sc13);
				
				//arr3의 값(새로 추가된 정보)를 출력해보고, 업데이트 된 arr1의 갯수를 확인 
				System.out.println("==========================================");
		 		System.out.println("추가된 상품의 정보입니다. ");
		 		System.out.println(arr3.get(arr3.size()-1));
...
//        System.out.println("==========================================");
	...
	//1) 상품 입력 메소드 - 인자값을 받고 연산 뒤 다시 리턴하는 방식 ///////////////////////
	private static ArrayList<SMarket>  kajaCreate(ArrayList<SMarket> arr1, String sc11, int sc12, String sc13) {
		System.out.println("---------------상품을 입력하세요 ---------------");
		System.out.println("현재 총 "+(arr1.size())+"개 입니다. "); //받아온 인자 arr1으로 게산해봄 
 		
 		arr1.add(new SMarket(sc11, sc12, sc13));//main 입력 받은 값을 인자로 받아 arr1에 넣는 작업 (업데이트 됨)  
        
        ArrayList<SMarket> arr2=new ArrayList<SMarket>(); // 리턴할 값을 담을 새 그릇
        for (int i = 0; i < arr1.size(); i++) { 
        	arr2.add(arr1.get(i)); //리턴할 새그릇에 넣기 
		}
		return arr2; //값 반환
