	...
	public void run() {
		...
		try {
			while(true) {
				...
				PaintFrameGUI.p2X=(int)PaintFrameGUI.p2.getLocationOnScreen().getX(); //좌표 변수를 만들고  static으로 변경
				PaintFrameGUI.p2Y=(int)PaintFrameGUI.p2.getLocationOnScreen().getY();
				//스크린 캡쳐 받아 이미지 버퍼에 저장 
				imageSendC = robot.createScreenCapture(new Rectangle(PaintFrameGUI.p2X-0, PaintFrameGUI.p2Y-400, 400, 400));//스크린샷을 찍어서 image에 저장해
