...
class ServerClass {
	...
	ArrayList<ThreadServerClass> threadList = new ArrayList<ThreadServerClass>(); //제네릭 접속 유저에 대한 배열 
	...
	// sendChat() 메소드 정의문 	// 접속 중인 쓰레드에게 chat 내용 보낸다.
	...
	// 이미지 전달 
	public void sendImage(String chat) throws IOException {
		for (int i=0; i<threadList.size(); i++)
			threadList.get(i).outputStream.writeUTF(chat);
	}
	...
	class ThreadServerClass extends Thread {
		...
		public void run() { //run 쓰레드 실한 구간 
			String nickname = "";
			try {
				if(inputStream != null) {
					nickname = inputStream.readUTF(); //입력 스트림으로 받은 문자열에 닉네임 대입 
					sendChat(nickname + " 님 입장 !!!(^^)"); //sendChat으로 전체 유저에게 내용 날림 
				}
				...
				for (int i = 0; i < threadList.size(); i++) {
					if(socket.equals(threadList.get(i).s1)) {
						threadList.remove(i);
						//===============
						try {
